...
from logwatch.collector.scheduler import (
...
from logwatch.notify.wecom import WecomNotifier, build_wecom_webhook_sender
from logwatch.web.api import ReloadAction, create_api_router
from logwatch.web.chat import WsTicketStore, create_chat_router
from logwatch.web.frontend import mount_frontend
...
DEFAULT_METRICS_DB_PATH = "data/logwatch.db"
...
def create_app(
    *,
            ...
            *,
            ...
            if collect_host_metrics_fn is not None:
                ...
                collect_host_metrics_module = importlib.import_module(
                    "logwatch.collector.host_metrics_probe"
                )
            ...
            if security_enabled:
                assess_host_security_impl = getattr(
                    importlib.import_module("logwatch.collector.host_metrics_probe"),
    ...
    app = FastAPI(title="LogWatch", lifespan=lifespan_handler)
...
def _resolve_app_config(
    *,
    ...
    has_env_config = os.getenv("LOGWATCH_CONFIG", "").strip() != ""
    if (
        config_path is None
...
def _resolve_metrics_db_path(
    ...
    env_path = os.getenv("LOGWATCH_METRICS_DB_PATH")
    if env_path:
        return env_path
    if config_db_path:
